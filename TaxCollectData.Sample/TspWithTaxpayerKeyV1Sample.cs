...
public class TspWithTaxpayerKeyV1Sample
{
    ...
    private readonly IProperties _properties = new NormalProperties(ClientType.TSP, "v1");
    ...
    public TspWithTaxpayerKeyV1Sample()
    {
    ...
    private async Task SendInvoicesAsync() {
        ...
        var serializedInvoice = Encoding.UTF8.GetString(JsonSerializer.SerializeToUtf8Bytes(invoiceDto, s_jsonSerializerOptions));
        
        var normalizer = new SimpleNormalizer();
        var normalize = normalizer.Normalize(serializedInvoice, null);
